...
- Печать
  ...
  - Перезапуск службы Диспетчер печати, очистка очереди печати, или применение всех исправлений разом
...
- Сеть
  - Очистка/регистрация DNS, сброс сети (Winsock/IP), полный сетевой отчёт в файл и авто‑открытие в Notepad
- Полная диагностика
...
- Работа с 1С
  - Очистка локального кэша пользователей (AppData\Local\1C\1Cv8/1Cv82 для всех пользователей)
  - Глубокая очистка кэша (ОСТОРОЖНО!!!) — требует явного подтверждения; очищает Local и Roaming; затрагивает настройки рабочего места и привязку оборудования
  - Очистка серверного кэша 1С — остановка службы агента (RU/EN имя определяется автоматически), очистка каталогов `snccntx*` во всех `srvinfo\reg_*` под `%ProgramFiles%\1cv8` и, при наличии, `%ProgramFiles(x86)%\1cv8`; при отсутствии стандартных путей будет запрошен путь; затем запуск службы
...
ВНИМАНИЕ по разделу «Работа с 1С»:
- «Глубокая очистка кэша» — затрагивает локальные настройки и привязку оборудования; требует явного подтверждения (ввести «ДА»).
- «Очистка серверного кэша» — временно останавливает службу агента 1С, удаляет серверные кэши и возвращает службу в рабочее состояние; убедитесь, что это допустимо в вашей среде.
